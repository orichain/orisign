...
static inline void quat_lattice_init(quat_lattice_t *lat) {
  ibz_mat_4x4_init(&(*lat).basis);
  ibz_init(&(*lat).denom);
  ibz_set(&(*lat).denom, 1);
}

static inline void quat_lattice_finalize(quat_lattice_t *lat) {
  ibz_finalize(&(*lat).denom);
  ibz_mat_4x4_finalize(&(*lat).basis);
}

static inline void quat_left_ideal_init(quat_left_ideal_t *lideal) {
  quat_lattice_init(&(*lideal).lattice);
  ibz_init(&(*lideal).norm);
  (*lideal).parent_order = NULL;
}

static inline void quat_left_ideal_finalize(quat_left_ideal_t *lideal) {
  ibz_finalize(&(*lideal).norm);
  quat_lattice_finalize(&(*lideal).lattice);
}

static inline void quat_alg_elem_init(quat_alg_elem_t *elem) {
  ibz_vec_4_init(&(*elem).coord);
  ibz_init(&(*elem).denom);
  ibz_set(&(*elem).denom, 1);
}

static inline void quat_alg_elem_finalize(quat_alg_elem_t *elem) {
  ibz_vec_4_finalize(&(*elem).coord);
  ibz_finalize(&(*elem).denom);
}

static inline void quat_alg_coord_mul(ibz_vec_4_t *res, const ibz_vec_4_t *a, const ibz_vec_4_t *b, const quat_alg_t *alg) {
  ibz_t prod;
  ibz_vec_4_t sum;
  ibz_init(&prod);
  ibz_vec_4_init(&sum);
...
static inline void quat_alg_mul(quat_alg_elem_t *res, const quat_alg_elem_t *a, const quat_alg_elem_t *b, const quat_alg_t *alg) {
  ibz_mul(&(res->denom), &(a->denom), &(b->denom));
  quat_alg_coord_mul(&(res->coord), &(a->coord), &(b->coord), alg);
}

static inline void quat_alg_conj(quat_alg_elem_t *conj, const quat_alg_elem_t *x) {
...
static inline void quat_alg_norm(ibz_t *res_num, ibz_t *res_denom, const quat_alg_elem_t *a, const quat_alg_t *alg) {
  ibz_t r, g;
...
static inline int quat_alg_elem_is_zero(const quat_alg_elem_t *x) {
  int res = ibz_vec_4_is_zero(&(x->coord));
  return (res);
}

static inline void quat_alg_scalar(quat_alg_elem_t *elem, const ibz_t *numerator, const ibz_t *denominator) {
...
static inline void quat_alg_equal_denom(quat_alg_elem_t *res_a, quat_alg_elem_t *res_b, const quat_alg_elem_t *a, const quat_alg_elem_t *b) {
  ibz_t gcd, r;
  ibz_init(&gcd);
  ibz_init(&r);
...
static inline void quat_alg_add(quat_alg_elem_t *res, const quat_alg_elem_t *a, const quat_alg_elem_t *b) {
...
static inline void quat_alg_normalize(quat_alg_elem_t *x) {
  ibz_t gcd, sign, r;
  ibz_init(&gcd);
  ibz_init(&sign);
  ibz_init(&r);
...
static inline void quat_order_elem_create(quat_alg_elem_t *elem, const quat_p_extremal_maximal_order_t *order, const ibz_vec_4_t *coeffs, const quat_alg_t *Bpoo) {
...
static inline int quat_lattice_contains(ibz_vec_4_t *coord, const quat_lattice_t *lat, const quat_alg_elem_t *x) {
  int divisible = 0;
...
static inline void quat_alg_make_primitive(ibz_vec_4_t *primitive_x, ibz_t *content, const quat_alg_elem_t *x, const quat_lattice_t *order) {
...
static inline int quat_represent_integer(quat_alg_elem_t *gamma, const ibz_t *n_gamma, int non_diag, const quat_represent_integer_params_t *params) {
  ...
  ibz_vec_4_t coeffs;
  quat_alg_elem_t quat_temp;
  if (non_diag) assert(params->order->q % 4 == 1);
  found = 0;
  ibz_init(&bound);
  ibz_init(&test);
  ibz_init(&temp);
  ...
  while (!found && ibz_cmp(&counter, &ibz_const_zero) != 0) {
    ...
    if (ibz_probab_prime(&cornacchia_target, params->primality_test_iterations)) found = ibz_cornacchia_prime(&(coeffs[0]), &(coeffs[1]), &q, &cornacchia_target);
    else found = 0;
    if (found && non_diag && standard_order) {
      if (ibz_is_odd(&coeffs[0]) != ibz_is_odd(&coeffs[3])) {
        ibz_swap(&coeffs[1], &coeffs[0]);
      }
      found = found && ((ibz_get(&coeffs[0]) - ibz_get(&coeffs[3])) % 4 == 2) && ((ibz_get(&coeffs[1]) - ibz_get(&coeffs[2])) % 4 == 2);
    }
    if (found) {
      ...
      if (non_diag || standard_order) found = (ibz_cmp(&temp, &ibz_const_two) == 0);
      else found = (ibz_cmp(&temp, &ibz_const_one) == 0);
    }
  }
  if (found) {
...
static inline int quat_order_discriminant(ibz_t *disc, const quat_lattice_t *order, const quat_alg_t *alg) {
  int ok = 0;
  ibz_t det, sqr, div;
...
static inline int quat_order_is_maximal(const quat_lattice_t *order, const quat_alg_t *alg) {
  int res;
  ibz_t disc;
  ibz_init(&disc);
...
static inline void quat_lattice_reduce_denom(quat_lattice_t *reduced, const quat_lattice_t *lat) {
  ibz_t gcd;
  ibz_init(&gcd);
...
static inline void quat_lattice_hnf(quat_lattice_t *lat) {
  ibz_t mod;
  ibz_vec_4_t generators[4];
  ibz_init(&mod);
  ibz_mat_4x4_inv_with_det_as_denom(NULL, &mod, &(lat->basis));
  ibz_abs(&mod, &mod);
  for (int i = 0; i < 4; i++) ibz_vec_4_init(&(generators[i]));
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
  ...
  ibz_finalize(&mod);
  for (int i = 0; i < 4; i++) ibz_vec_4_finalize(&(generators[i]));
}

static inline void quat_lattice_mat_alg_coord_mul_without_hnf(ibz_mat_4x4_t *prod, const ibz_mat_4x4_t *lat, const ibz_vec_4_t *coord, const quat_alg_t *alg) {
  ibz_vec_4_t p, a;
  ibz_vec_4_init(&p);
  ibz_vec_4_init(&a);
  for (int i = 0; i < 4; i++) {
...
static inline void quat_lattice_alg_elem_mul(quat_lattice_t *prod, const quat_lattice_t *lat, const quat_alg_elem_t *elem, const quat_alg_t *alg) {
...
static inline void quat_lideal_create_principal(quat_left_ideal_t *lideal, const quat_alg_elem_t *x, const quat_lattice_t *order, const quat_alg_t *alg) {
...
static inline void quat_lattice_add(quat_lattice_t *res, const quat_lattice_t *lat1, const quat_lattice_t *lat2) {
  ...
  ibz_init(&det2);
  ibz_init(&detprod);
  for (int i = 0; i < 8; i++) ibz_vec_4_init(&(generators[i]));
  ...
  for (int i = 0; i < 8; i++) ibz_vec_4_finalize(&(generators[i]));
}

static inline void quat_lattice_index(ibz_t *index, const quat_lattice_t *sublat, const quat_lattice_t *overlat) {
  ibz_t tmp, det;
  ibz_init(&tmp);
  ibz_init(&det);
...
static inline void quat_lideal_norm(quat_left_ideal_t *lideal) {
...
static inline void quat_lideal_create(quat_left_ideal_t *lideal, const quat_alg_elem_t *x, const ibz_t *N, const quat_lattice_t *order, const quat_alg_t *alg) {
  assert(quat_order_is_maximal(order, alg));
...
static inline int quat_sampling_random_ideal_O0_given_norm(quat_left_ideal_t *lideal, const ibz_t *norm, int is_prime, const quat_represent_integer_params_t *params, const ibz_t *prime_cofactor) {
  ibz_t n_temp, norm_d;
  ibz_t disc;
  ...
  if (is_prime) {
    while (!found) {
      ...
      for (int i = 1; i < 4; i++) ibz_rand_interval(&gen.coord[i], &ibz_const_zero, &n_temp);
...
static inline void quat_lattice_gram(ibz_mat_4x4_t *G, const quat_lattice_t *lattice, const quat_alg_t *alg) {
  ibz_t tmp;
  ibz_init(&tmp);
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j <= i; j++) {
...
static inline void quat_lideal_class_gram(ibz_mat_4x4_t *G, const quat_left_ideal_t *lideal, const quat_alg_t *alg) {
...
static inline void quat_lll_core(ibz_mat_4x4_t *G, ibz_mat_4x4_t *basis) {
...
static inline void quat_lideal_reduce_basis(ibz_mat_4x4_t *reduced, ibz_mat_4x4_t *gram, const quat_left_ideal_t *lideal, const quat_alg_t *alg) {
...
static inline int quat_lideal_norm_verify(const quat_left_ideal_t *lideal) {
  int res;
  ibz_t index;
  ibz_init(&index);
...
static inline void quat_lideal_mul(quat_left_ideal_t *product, const quat_left_ideal_t *lideal, const quat_alg_elem_t *alpha, const quat_alg_t *alg) {
  assert(quat_order_is_maximal((lideal->parent_order), alg));
  ibz_t norm, norm_d;
  ibz_init(&norm);
  ibz_init(&norm_d);
...
static inline void quat_qf_eval(ibz_t *res, const ibz_mat_4x4_t *qf, const ibz_vec_4_t *coord) {
  ibz_vec_4_t sum;
  ibz_t prod;
  ibz_init(&prod);
  ibz_vec_4_init(&sum);
...
static inline int quat_lideal_prime_norm_reduced_equivalent(quat_left_ideal_t *lideal, const quat_alg_t *alg, const int primality_num_iter, const int equiv_bound_coeff) {
...
static inline void quat_lideal_copy(quat_left_ideal_t *copy, const quat_left_ideal_t *copied) {
...
static inline void quat_lattice_conjugate_without_hnf(quat_lattice_t *conj, const quat_lattice_t *lat) {
...
static inline void quat_lideal_inverse_lattice_without_hnf(quat_lattice_t *inv, const quat_left_ideal_t *lideal, const quat_alg_t *alg) {
  assert(quat_order_is_maximal((lideal->parent_order), alg));
  quat_lattice_conjugate_without_hnf(inv, &(lideal->lattice));
  ibz_mul(&(inv->denom), &(inv->denom), &(lideal->norm));
}

static inline void quat_lattice_mul(quat_lattice_t *res, const quat_lattice_t *lat1, const quat_lattice_t *lat2, const quat_alg_t *alg) {
  ...
  for (int i = 0; i < 16; i++) ibz_vec_4_finalize(&(generators[i]));
}

static inline void quat_lideal_right_transporter(quat_lattice_t *trans, const quat_left_ideal_t *lideal1, const quat_left_ideal_t *lideal2, const quat_alg_t *alg) {
...
static inline void quat_lideal_right_order(quat_lattice_t *order, const quat_left_ideal_t *lideal, const quat_alg_t *alg) {
  assert(quat_order_is_maximal((lideal->parent_order), alg));
  quat_lideal_right_transporter(order, lideal, lideal, alg);
}

static inline void quat_lideal_conjugate_without_hnf(quat_left_ideal_t *conj, quat_lattice_t *new_parent_order, const quat_left_ideal_t *lideal, const quat_alg_t *alg) {
...
static inline void quat_lideal_lideal_mul_reduced(quat_left_ideal_t *prod, ibz_mat_4x4_t *gram, const quat_left_ideal_t *lideal1, const quat_left_ideal_t *lideal2, const quat_alg_t *alg) {
  ibz_mat_4x4_t red;
  ibz_mat_4x4_init(&red);
...
static inline void quat_alg_elem_copy(quat_alg_elem_t *copy, const quat_alg_elem_t *copied) {
...
static inline int quat_lideal_generator(quat_alg_elem_t *gen, const quat_left_ideal_t *lideal, const quat_alg_t *alg) {
...
static inline void quat_lattice_dual_without_hnf(quat_lattice_t *dual, const quat_lattice_t *lat) {
  ibz_mat_4x4_t inv;
  ibz_t det;
  ibz_init(&det);
  ibz_mat_4x4_init(&inv);
...
static inline void quat_lattice_intersect(quat_lattice_t *res, const quat_lattice_t *lat1, const quat_lattice_t *lat2) {
...
static inline void quat_lideal_inter(quat_left_ideal_t *inter, const quat_left_ideal_t *I1, const quat_left_ideal_t *I2, const quat_alg_t *alg) {
...
static inline int quat_lattice_bound_parallelogram(ibz_vec_4_t *box, ibz_mat_4x4_t *U, const ibz_mat_4x4_t *G, const ibz_t *radius) {
  ibz_t denom, rem;
  ibz_init(&denom);
  ibz_init(&rem);
  ibz_mat_4x4_t dualG;
...
static inline int quat_lattice_sample_from_ball(quat_alg_elem_t *res, const quat_lattice_t *lattice, const quat_alg_t *alg, const ibz_t *radius) {
...
static inline void quat_change_to_O0_basis(ibz_vec_4_t *vec, const quat_alg_elem_t *el) {
  ibz_t tmp;
  ibz_init(&tmp);
