...
// double-wide multiplication
  void
MUL(uint64_t *out, const uint64_t a, const uint64_t b)
{
#ifdef RADIX_32
  ...
  ah = a >> (sizeof(uint64_t) * 4); // High part
  bl = b & mask_low;
  ...
  out[0] ^= temp << (sizeof(uint64_t) * 4); // out01
  ...
  void
mp_add(uint64_t *c, const uint64_t *a, const uint64_t *b, const unsigned int nwords)
{ // Multiprecision addition
  ...
  uint64_t
mp_shiftr(uint64_t *x, const unsigned int shift, const unsigned int nwords)
{ // Multiprecision right shift by 1...RADIX-1
  ...
  void
mp_shiftl(uint64_t *x, const unsigned int shift, const unsigned int nwords)
{ // Multiprecision left shift by 1...RADIX-1
  ...
  void
multiple_mp_shiftl(uint64_t *x, const unsigned int shift, const unsigned int nwords)
{
  int t = shift;
  while (t > RADIX - 1) {
...
// The below functions were taken from the EC module
  ...
  void
mp_sub(uint64_t *c, const uint64_t *a, const uint64_t *b, const unsigned int nwords)
{ // Multiprecision subtraction, assuming a > b
  ...
  void
select_ct(uint64_t *c, const uint64_t *a, const uint64_t *b, const uint64_t mask, const int nwords)
{ // Select c <- a if mask = 0, select c <- b if mask = 1...1
  ...
  void
swap_ct(uint64_t *a, uint64_t *b, const uint64_t option, const int nwords)
{ // Swap entries
  ...
  int
mp_compare(const uint64_t *a, const uint64_t *b, unsigned int nwords)
{ // Multiprecision comparison, a=b? : (1) a>b, (0) a=b, (-1) a<b
  ...
  bool
mp_is_zero(const uint64_t *a, unsigned int nwords)
{ // Is a multiprecision element zero?
  ...
  void
mp_mul2(uint64_t *c, const uint64_t *a, const uint64_t *b)
{ // Multiprecision multiplication fixed to two-digit operands
  ...
  void
mp_print(const uint64_t *a, size_t nwords)
{
  printf("0x");
  ...
  void
mp_copy(uint64_t *b, const uint64_t *a, size_t nwords)
{
  for (size_t i = 0; i < nwords; i++) {
    b[i] = a[i];
  }
  ...
  void
mp_mul(uint64_t *c, const uint64_t *a, const uint64_t *b, size_t nwords)
{
  ...
  void
mp_mod_2exp(uint64_t *a, unsigned int e, unsigned int nwords)
{ // Multiprecision modulo 2^e, with 0 <= a < 2^(e)
  ...
  void
mp_neg(uint64_t *a, unsigned int nwords)
{ // negates a
  for (size_t i = 0; i < nwords; i++) {
    a[i] ^= -1;
  }
  ...
  bool
mp_is_one(const uint64_t *x, unsigned int nwords)
{ // returns true if x represents 1, and false otherwise
  ...
  void
mp_inv_2e(uint64_t *b, const uint64_t *a, int e, unsigned int nwords)
{ // Inversion modulo 2^e, using Newton's method and Hensel lifting
  ...
  void
mp_invert_matrix(uint64_t *r1, uint64_t *r2, uint64_t *s1, uint64_t *s2, int e, unsigned int nwords)
{
